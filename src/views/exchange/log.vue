<script setup>
import dayjs from 'dayjs'
import { showFailToast } from 'vant'
import { contractList } from '@/api/contract'
import { cancelExchange, exchangeList } from '@/api/exchange'

const onClickLeft = () => history.back()

const active = ref(0)

const historyList = ref([])
const loading = ref(false)
const getData = async () => {
  loading.value = true
  const { code, data } = await exchangeList(active.value)
  historyList.value = data
  loading.value = false
}

const changeActive = () => {
  historyList.value = []
  getData()
}

async function cancel(id) {
  const { code, data, msg } = await cancelExchange(id)
  if (code !== 1) {
    showFailToast(msg)
    return false
  }
  await getData()
}

onBeforeMount(async () => {
  await getData()
})
</script>

<template>
  <div>
    <van-nav-bar
      @click-left="onClickLeft"
    >
      <template #title>
        <span class="text-[#121935]">交易订单</span>
      </template>
      <template #left>
        <van-icon name="arrow-left" color="#121935" />
      </template>
    </van-nav-bar>
    <van-tabs v-model:active="active" shrink @change="changeActive">
      <van-tab title="当前委托">
        <div v-loading="loading" class="w-full bg-white mt-4 rounded-t-xl min-h-[100vh] p-[20px]">
          <van-empty v-if="historyList.length === 0" description="暂无数据" />
          <div v-else class="flex flex-col order-list">
            <div v-for="item in historyList" :key="item" class="flex flex-col mb-4 order-item">
              <div class="flex flex-row items-center justify-between">
                <div class="flex flex-row items-center">
                  <div class="py-1 px-2 rounded text-[14px] mr-1" :class="item.direction === '0' ? 'bgreen' : 'bred'">
                    {{ item.direction_text }}
                  </div>
                  <span class="text-[#343743]">{{ item.coin.title }}</span>
                </div>
                <van-button size="small" type="primary" @click="cancel(item.id)">
                  取消委托
                </van-button>
              </div>
              <div class="text-[10px] mt-1 mb-3">
                <span class="text-[#888B91]">{{ item.createtime_text }}</span>
              </div>
              <div class="flex flex-row items-center justify-between text-[10px] mb-2">
                <span class="text-[#888B91]">委托价格</span>

                <span class="text-[#282B38]">{{ item.entrust_price }}</span>
              </div>
              <div class="flex flex-row items-center justify-between text-[10px] mb-2">
                <span class="text-[#888B91]">委托数量</span>

                <span class="text-[#282B38]">{{ item.amount }}</span>
              </div>
              <div class="flex flex-row items-center justify-between text-[10px] mb-2">
                <span class="text-[#888B91]">成交价格</span>

                <span class="text-[#282B38]">--</span>
              </div>
              <hr class="text-[#F7F8F9]">
            </div>
          </div>
        </div>
      </van-tab>
      <van-tab title="历史委托">
        <div v-loading="loading" class="w-full bg-white mt-4 rounded-t-xl min-h-[100vh] p-[20px]">
          <van-empty v-if="historyList.length === 0" description="暂无数据" />
          <div v-else class="flex flex-col order-list">
            <div v-for="item in historyList" :key="item.id" class="flex flex-col mb-4 order-item">
              <div class="flex flex-row items-center">
                <div class="py-1 px-2 rounded text-[14px] mr-1" :class="item.direction === '0' ? 'bgreen' : 'bred'">
                  {{ item.direction_text }}
                </div>
                <span class="text-[#343743]">{{ item.coin.title }}</span>
              </div>
              <div class="text-[10px] mt-1 mb-3">
                <span class="text-[#888B91]">{{ item.createtime_text }}</span>
              </div>
              <div class="flex flex-row items-center justify-between text-[10px] mb-2">
                <span class="text-[#888B91]">成交数量</span>

                <span class="text-[#282B38]">{{ item.amount }}</span>
              </div>  <div class="flex flex-row items-center justify-between text-[10px] mb-2">
                <span class="text-[#888B91]">成交价格</span>

                <span class="text-[#282B38]">{{ item.entrust_price }}</span>
              </div>
              <div class="flex flex-row items-center justify-between text-[10px] mb-4">
                <span class="text-[#888B91]">成交时间</span>

                <span class="text-[#282B38]">{{ item.updatetime_text }}</span>
              </div>
              <hr class="text-[#F7F8F9]">
            </div>
          </div>
        </div>
      </van-tab>
    </van-tabs>
  </div>
</template>

<style lang="scss">
.van-tabs__nav,.van-nav-bar {
  background: transparent;
}
.order-item:last-child {
  hr {
    opacity: 0;
  }
}
</style>
